        ...
        if Type == WHOHAS:
            ...
            if ack_num * MAX_PAYLOAD >= CHUNK_DATA_SIZE:
                ...
                if ack_num >= self.sendWin_lower[Team]:
                    ...
                    if self.congWinSize[Team] >= self.ssthreshold[Team] and self.congState[Team] == SLOW_START:
                    ...
                    if self.dupACKcount[Team] == 4:
                        ...
                        if self.congState[Team] == CONGECTION_AVOIDANCE:
                            self.congState[Team] = SLOW_START
                        print('！！！！！！！！！！！！！！！！！发生冗余重传啦！！！！！！！！！！！！！！！！！！！！！')
                        print(f'origin threshold: {self.ssthreshold[Team]}')
                        self.ssthreshold[Team] = max(math.floor(self.congWinSize[Team] / 2), 2)
                        self.congWinSize[Team] = 1
                        self.congWinSizeChange[Team].append([round(time() - self.globelTimer, 2),round(self.congWinSize[Team],2)])
                        print(f'after change threshold: {self.ssthreshold[Team]}')
                        print(f'congWinSizeChange add: {[round(time() - self.globelTimer, 2),round(self.congWinSize[Team],2)]}')
                        print('冗余ACK, congWin = 1')
...
def peer_run(configuration):
    ...
    try:
        ...
        for p in peer.congWinSizeChange:
            print(peer.congWinSizeChange[p])
            data = np.array(peer.congWinSizeChange[p])
            x = data[:,0]
            y = data[:,1]
            plt.scatter(x, y, s=1)
            plt.savefig(f'team: {peer.team}, congection-window of team: {p}.png')
    finally:
        sock.close()
